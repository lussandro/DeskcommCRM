 ...
 * aprendeu a escrever template no Inbox escreve igual aqui. As duas que a
 * campanha acrescenta existem porque há dado para elas — `{{empresa}}` sai de
 * `crm_companies` (0260) e `{{saudacao}}` do relógio.
 *
...
export const VARIAVEIS_DA_CAMPANHA = ["nome", "primeiro_nome", "empresa", "saudacao"] as const;
...
export const DESCRICAO_DA_VARIAVEL: Record<VariavelDaCampanha, string> = {
  ...
  empresa: "Nome da empresa vinculada ao contato",
  ...
  const texto = template.replace(TOKEN, (literal, bruto: string) => {
    ...
    switch (chave) {
      ...
      case "empresa": {
        ...
        if (empresa === "") return marcarFalta(faltando, "empresa", literal);
        return empresa;
      }
      case "saudacao": {
