...
describe("renderizador da campanha", () => {
  ...
  it("tolera espaço dentro do token", () => {
    const r = renderizar("Oi {{  primeiro_nome  }}", { nome: "João Silva", empresa: null });
    expect(r.texto).toBe("Oi João");
  });

  it("não envia texto com buraco: variável sem valor volta como FALTANDO e o literal fica", () => {
    const r = renderizar("Olá {{nome}}, tudo bem?", { nome: "   ", empresa: null });
  ...
  it("empresa vazia também é falta — a mensagem que cita a empresa sem citá-la denuncia o disparo", () => {
    const r = renderizar("Falo da {{empresa}}?", { nome: "Ana", empresa: null });
    expect(r.faltando).toEqual(["empresa"]);
  });

  it("token desconhecido fica literal e é reportado, nunca vira vazio", () => {
    const r = renderizar("Oi {{sobrenome}}", { nome: "Ana Souza", empresa: null });
  ...
  it("texto sem token passa intacto, unicode incluído", () => {
    ...
    expect(renderizar(t, { nome: null, empresa: null }).texto).toBe(t);
  });

  it("a saudação é a da HORA DO ENVIO, não a do texto", () => {
    const manha = renderizar("{{saudacao}}!", { nome: null, empresa: null }, { agora: MANHA, fuso: FUSO });
    const tarde = renderizar("{{saudacao}}!", { nome: null, empresa: null }, { agora: TARDE, fuso: FUSO });
  ...
  it("sem instante (prévia) a saudação fica literal — a prévia não inventa a hora do envio", () => {
    const r = renderizar("{{saudacao}}!", { nome: null, empresa: null });
  ...
  it("lista as variáveis que o texto usa, sem repetir e sem inventar", () => {
    expect(variaveisUsadas("{{nome}} e {{nome}} e {{empresa}} e {{xpto}}").sort()).toEqual([
      "empresa",
      "nome",
    ]);
  ...
  it("não executa nada: chave com sintaxe de caminho não atravessa propriedade", () => {
    const r = renderizar("{{constructor.name}} {{__proto__}}", { nome: "Ana", empresa: null });
