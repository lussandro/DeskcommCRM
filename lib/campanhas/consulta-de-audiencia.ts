...
export async function buscarCandidatos(
  ...
  let consulta = admin
    .from("contacts")
    .select("id, name, display_name, phone_number, is_blocked, is_anonymized, consent, company_id")
    .eq("organization_id", organizationId)
  ...
  const empresas = await nomesDasEmpresas(admin, organizationId, linhas);

  return linhas.map((l) => ({
    contactId: l.id,
    nome: l.name ?? l.display_name,
    empresa: l.company_id ? (empresas.get(l.company_id) ?? null) : null,
...
async function nomesDasEmpresas(
  ...
  linhas: readonly LinhaDeContato[],
): Promise<Map<string, string>> {
  const ids = [...new Set(linhas.map((l) => l.company_id).filter((id): id is string => !!id))];
  ...
  const { data, error } = await admin
    ...
    .eq("organization_id", organizationId)
    .in("id", ids);
  if (error) throw new Error(`audiência: empresas — ${error.message}`);
  return new Map((data ?? []).map((e) => [(e as { id: string }).id, (e as { name: string }).name]));
}
