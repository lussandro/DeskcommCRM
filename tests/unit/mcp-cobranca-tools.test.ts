...
vi.mock("@/lib/agent-engine/db/request-pool", () => ({
  getRequestPool: vi.fn(() => ({ query: vi.fn().mockResolvedValue({ rows: [{ timezone: null }] }) })),
}));
...
describe("crm_reissue_overdue_charge", () => {
  ...
  it("reissue_count >= max → recusa { refused: 'limite', vezes }", async () => {
    const integ = integracaoStub();
  ...
  it("OK → PUT com dueDate = hoje+dias, upsert em asaas_charges, insert em asaas_charge_actions, atividade e audit com uuid resourceId e payment_id em metadata", async () => {
    const integ = integracaoStub();
    (integ.cliente.payment as ReturnType<typeof vi.fn>).mockResolvedValue({ id: PAYMENT, customer: CUSTOMER, status: "OVERDUE", value: 100, dueDate: "2026-08-01", billingType: "BOLETO" });
    ...
    const cap = novasCapturas();
    const resolver: Resolver = (c) => {
      if (c.table === "asaas_charges" && c.terminal === "maybeSingle") return { data: { reissue_count: 0 }, error: null };
      return contatoVinculado()(c);
    };

    const res = (await crmReissueOverdueCharge.handler({ contact_id: CONTATO, payment_id: PAYMENT }, ctxDe(resolver, cap))) as { new_due_date: string };
    ...
    expect(upsert?.values.reissue_count).toBe(1);
    ...
    expect(audit).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        resourceId: CONTATO,
        metadata: expect.objectContaining({ payment_id: PAYMENT }),
      }),
    );
