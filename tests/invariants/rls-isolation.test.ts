    ...
    begin
      foreach v_org in array array['${ORG_A}'::uuid, '${ORG_B}'::uuid] loop
        ...
        if not exists (select 1 from public.campaigns where organization_id = v_org) then
          insert into public.campaigns (organization_id, name, channel_session_id, template_body, base_legal, lia_ref)
            values (v_org, 'RLS Invariant Campanha', v_sess, 'Oi, tudo bem?', 'legitimate_interest', 'LIA-RLS')
            returning id into v_campanha;
          insert into public.campaign_recipients (organization_id, campaign_id, contact_id)
            values (v_org, v_campanha, v_contact);
        end if;
...
export const TABLES = [
  ...
  "org_voice_calls",
  // migration 0264 — campanha de prospecção. A lista de destinatários é o ativo
  // comercial mais sensível que existe aqui: quem vazasse leria a quem o
  // vizinho está prospectando e com que texto. A ESCRITA exige `admin` e NÃO é
  // medida neste arquivo (o usuário semeado é `agent`); quem mede é a rota.
  "campaigns",
  "campaign_recipients",
